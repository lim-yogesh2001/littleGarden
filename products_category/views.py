...
from .serializer import CategorySerializer, ProductSerialzier, ProductReviewSerializer, ProductWriteReviewSerializer
import traceback
    ...
    def get(self, request):
        try:
            products = Product.objects.filter(
                is_popular=False, is_recently_added=False)
            serializer = ProductSerialzier(products, many=True)
            return Response(serializer.data, status=status.HTTP_200_OK)
        except Product.DoesNotExist:
            return Response({"Products Not Found"}, status=status.HTTP_404_NOT_FOUND)
...
class ProductReviews(APIView):
    ...
    def post(self, request, product_id):
        try:
            product = Product.objects.get(id=product_id)
            serializer = ProductWriteReviewSerializer(data=request.data)
